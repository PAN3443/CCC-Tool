class class_Graph_ForcedDisPower_Distance extends class_Graph_ForcedLayout {

  forceLayout(iterations, optiDistanceDegree,degree,isGlobal){
    if(isGlobal){
      this.forceLayoutGlobal(iterations, optiDistanceDegree,degree);
    }
    else{
      this.forceLayoutLocal(iterations, optiDistanceDegree,degree);
    }
  }

  forceLayoutGlobal(iterations, optiDistanceDegree,degree) {
    ...
    var optiDistance=this.determineMaxSetting(true)*optiDistanceDegree;
          ...
          if(this.nodeArray[v].getColorID()==1){
            ...
            var startColor = this.getNodeColor(nodeStartID);
              ...
              if(distance<optiDistance){
                ...
                var cDif_Change = distanceDif * degree;
  ...
  forceLayoutLocal(iterations, optiDistanceDegree,degree) {
    ...
    var optiDistance=this.determineMaxSetting(false)*optiDistanceDegree;
...
/*forceLayout(iterations, power, degree) {
  ...
  for (var i = 0; i < iterations; i++) {
    ...
    for (var e = 0; e < this.edgeArray.length; e++) {
      ...
      var color_v_Node = this.getNodeColor(idV);
      var color_u_Node = this.getNodeColor(idU);
  ...
  for (var i = 0; i < iterations; i++) {
    ...
    for (var v = 0; v < this.nodeArray.length; v++) {
      ...
      for (var u = 0; u < this.nodeArray.length; u++) {
        ...
        var vec_d = [color_v_Node[1] - color_u_Node[1], color_v_Node[2] - color_u_Node[2], color_v_Node[3] - color_u_Node[3]]; // difference vector between the positions of the two vertices
        var vec_dN = vecNorm(vec_d);
        var vec_dL = vecLength(vec_d);
    ...
    for (var e = 0; e < this.edgeArray.length; e++) {
      var color_v_Node = this.getNodeColor(this.edgeArray[e].getNodeID1());
      var color_u_Node = this.getNodeColor(this.edgeArray[e].getNodeID2());
