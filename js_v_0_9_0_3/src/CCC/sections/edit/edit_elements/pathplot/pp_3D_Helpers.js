...
function draw3DLine(xPos, yPos, zPos, xPos2, yPos2, zPos2, isDashed) {
  ...
  if (isDashed) lineMaterial = new THREE.LineDashedMaterial({ color: linecolor, dashSize: 10, gapSize: 10, linewidth: 5.0 });
  ...
  geometry.vertices.push(new THREE.Vector3(xPos, yPos, zPos), new THREE.Vector3(xPos2, yPos2, zPos2));
...
function draw3DElement(colorHexStr, xPos, yPos, zPos, index, colorside, circle, mouseAboveKeyID, mouseGrappedColorSide) {
  ...
  if (circle) {
    ...
    if (index == mouseAboveKeyID && colorside == mouseGrappedColorSide) geometry = new THREE.SphereGeometry(bigcircleRad3D, 20, 20);
    else geometry = new THREE.SphereGeometry(circleRad3D, 20, 20);
    ...
    if (index == mouseAboveKeyID && colorside == mouseGrappedColorSide) geometry = new THREE.BoxGeometry(bigcircleRad3D * 2, bigcircleRad3D * 2, bigcircleRad3D * 2);
    else geometry = new THREE.BoxGeometry(circleRad3D * 2, circleRad3D * 2, circleRad3D * 2);
