...
function draw_FunctionTypeTresh(canvasID, exp, fctType ){
  ...
  var stepX =(endX-startX)/(numberSteps-1);
  var startY=canvas.height*0.85;
  var middleY=canvas.height*0.5;
  var endY=canvas.height*0.15;
  var yGrad = middleY-startY;
  context.lineWidth = lineWidth;
  context.strokeStyle = lineColor;
  ...
  switch (fctType) {
    case 0:
    /////// Draw Linear Surrounding
    context.lineTo(endX, endY);
    context.stroke();
      break;
      case 1:
      /////// Draw Flat Surrounding
      for (var i = 0; i <numberSteps; i++) {
        var xPos = startX+i*stepX;
        var currentY = (i*ratioStep)*2-1;
        ...
        if(currentY<0 && exp%2==0){
            ...
            yPos= middleY-yGrad*(Math.pow(currentY,exp));
      ...
      case 2:
        ...
        for (var i = 0; i <numberSteps; i++) {
          ...
          if(currentY>0){
            ...
            if (exp%2==0)
              yPos= endY-yGrad*(Math.pow(currentY-1,exp));
            else
              yPos= middleY+yGrad*(Math.pow(currentY-1,exp)+1);

          }
          else{
              yPos= startY+yGrad*(Math.pow(currentY+1,exp));
          }
...
function draw_FunctionTypeTresh_CoordinateSystem(context,canvasHight,canvasWidth){
    ...
    context.beginPath();
    context.moveTo(canvasWidth*0.1, canvasHight*0.85);
    context.lineTo(canvasWidth*0.95, canvasHight*0.85);
    context.stroke();
    context.beginPath();
    context.lineWidth = 1;
    context.setLineDash([5, 3]);
    context.moveTo(canvasWidth*0.1, canvasHight*0.5);
    context.lineTo(canvasWidth*0.95, canvasHight*0.5);
    context.stroke();
    context.lineWidth = arrowlineWidth;
    context.moveTo(canvasWidth*0.1, canvasHight*0.15);
    context.lineTo(canvasWidth*0.95, canvasHight*0.15);
    context.stroke();
    context.lineWidth = arrowlineWidth;
    context.setLineDash([]);

    context.fillText("m", canvasWidth*0.05, canvasHight*0.85);
    context.fillText("T", canvasWidth*0.05, canvasHight*0.5);
    context.fillText('M', canvasWidth*0.05, canvasHight*0.15);
